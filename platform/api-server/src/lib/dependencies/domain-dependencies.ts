...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/queues\//, domain: "queues" },
  { pattern: /\/routing\//, domain: "routing" },
  { pattern: /\/vendors\//, domain: "vendors" },
];
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      ...
      case "portfolio":
        ...
        break;
      case "queues":
        domainDeps = buildQueuesDomainModule(dynamoClient);
        break;
      case "routing":
        domainDeps = buildRoutingDomainModule(dynamoClient);
        break;
      case "vendors":
        domainDeps = buildVendorsDomainModule(dynamoClient);
        break;
      default:
        return;
    }
...
export interface DomainDependencies {
  ...
  queues: QueuesDomainModule;
  routing: RoutingDomainModule;
  vendors: VendorsDomainModule;
}
...
export function getQueuesDeps(
  request: FastifyRequest,
): DomainDependencies["queues"] {
  return getDomainDeps(request, "queues");
}
export function getRoutingDeps(
  request: FastifyRequest,
): DomainDependencies["routing"] {
  return getDomainDeps(request, "routing");
}
export function getVendorsDeps(
  request: FastifyRequest,
): DomainDependencies["vendors"] {
  return getDomainDeps(request, "vendors");
}
