...
import type { ReviewRepository } from "@ddd/services/routing";
 ...
 *
 * Entity Type: HUMAN_REVIEW
 * Entity Token: HUMAN_REVIEW
 * SK Prefix: HUMAN_REVIEW
 */
export class ReviewRepositoryDdb implements ReviewRepository {
  ...
  private readonly ENTITY_TYPE = "HUMAN_REVIEW";
  private readonly ENTITY_TOKEN = "HUMAN_REVIEW";
  private readonly SK_PREFIX = "HUMAN_REVIEW";
  ...
  private readonly ENTITY_TYPE_PK = "HUMAN_REVIEWS";
  ...
  async recordHumanReview(input: Parameters<ReviewRepository['recordHumanReview']>[0]): Promise<Awaited<ReturnType<ReviewRepository['recordHumanReview']>>> {
    const inputExt = input as Record<string, unknown>;

    if (!input) {
      throw new Error("Missing required parameter: input");
    }
    if (!(inputExt.id ?? inputExt.workItemId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.workItemId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.workItemId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.workItemId)} not found`);
    }
    return {
