/**
 * RightRepository - DynamoDB Implementation
 *
 ...
 * RightRepositoryDdb
 *
 * DDD: Infrastructure adapter implementing Right repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { RightRepository } from "@ddd/services/routing";
...
/**
 * RightRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: RIGHTS_RECORD
 * Entity Token: RIGHTS_RECORD
 * SK Prefix: RIGHTS_RECORD
 */
export class RightRepositoryDdb implements RightRepository {
  private readonly TABLE_NAME: string;
  // Metadata from x-dynamodb annotation - all values are dynamic
  private readonly ENTITY_TYPE = "RIGHTS_RECORD";
  private readonly ENTITY_TOKEN = "RIGHTS_RECORD";
  private readonly SK_PREFIX = "RIGHTS_RECORD";
  ...
  private readonly ENTITY_TYPE_PK = "RIGHTS_RECORDS";
  ...
  async getRightsRecord(input: Parameters<RightRepository['getRightsRecord']>[0]): Promise<Awaited<ReturnType<RightRepository['getRightsRecord']>>> {
    ...
    if (!(inputExt.assetId ?? inputExt.id ?? inputExt.workItemId)) {
      throw new Error("Missing required parameter: assetId");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.assetId ?? inputExt.id ?? inputExt.workItemId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.assetId ?? inputExt.id ?? inputExt.workItemId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.assetId ?? inputExt.id ?? inputExt.workItemId)} not found`);
    }
    return {
